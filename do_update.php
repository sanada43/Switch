	...
	if (!preg_match("/^[0-9]+$/", $id)) {
		...
		try {
			...
			$stmt = $mysqli->prepare('SELECT id FROM schedules WHERE id<>? ' .
				'AND room_id=? AND sdate < ? AND edate > ? AND static=0');
			if ($stmt) {
				$stmt->bind_param("iiss", $id, $room_id, $edate, $sdate);
				$stmt->execute();
				$stmt->store_result();
				if ($stmt->fetch()) {
			...
			$stmt = $mysqli->prepare('UPDATE schedules SET ' .
				'sdate=?, edate=?, network_id=?, room_id=?, applicant=?, ' .
				'purpose=?, udate=now() WHERE id=? AND static=0');
			if ($stmt) {
				$stmt->bind_param("ssiissi", $sdate, $edate, $network_id,
					$room_id, $applicant, $purpose, $id);
				$stmt->execute();
				if ($stmt->affected_rows != 1) {
					$error_data['error_message'] = "変更に失敗しました。2";
					$error_data['id'] = $id;
					session_start();
					$_SESSION['error_data'] = $error_data;
					$stmt->close();
